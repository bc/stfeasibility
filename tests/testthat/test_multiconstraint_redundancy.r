...
test_that("H matrix of two moments with defined task on rhs has 0 sd in wrench_outputs",
    {
        ...
        solutions <- constr_tuple %>% eliminate_redundant %>% pb_har_sample(10000,
            mc.cores = 8, eliminate = FALSE) %>% as.matrix

        context("bound requirements")
        expect_true(all(as.numeric(unlist(solutions)) <= 1))
        expect_true(all(as.numeric(unlist(solutions)) >= 0))
...
test_that("we can make a mini-H multiconstraint with RHS", {
    ...
    # add the lambda=1 constraint: devtools::use_data(mini_trajectory_constr,
    # overwrite=TRUE)
})
...
test_that("we can make a mini-H multiconstraint", {
    ...
    mini_trajectory_constr <- diagonal_merge_constraint_list(constraint_list)
    expect_equal(ncol(mini_trajectory_constr$constr), length(mini_H_task_trajectory) *
        4)
...
test_that("zero variance occurs in lambda when we have fixed tasks per moment of a trajectory",
    {
        har_mini_task <- mini_trajectory_constr %>% eliminate_redundant %>% pb_har_sample(10000,
            mc.cores = 8, eliminate = FALSE)
        lambda_indices <- colnames(har_mini_task)[4 * (1:7)]
        variances_for_lambda_parameters <- apply(har_mini_task[, lambda_indices],
            2, sd) %>% as.numeric
        expect_equal(variances_for_lambda_parameters, rep(0, 7), 1e-10)
    })

test_that("adding lambda_q", {
    mini_trajectory_lambda_constr <- mini_trajectory_constr %>% add_lambda_equality_constraint(3)
    mini_trajectory_lambda_velocity_consr <- mini_trajectory_lambda_constr %>% generate_and_add_velocity_constraint(0.5,
        0.5, 3)
    full_constr_mini <- mini_trajectory_lambda_velocity_consr %>% eliminate_redundant

    expect_equal(ncol(mini_trajectory_constr_nonredundant$constr), length(mini_H_task_trajectory) *
        4)

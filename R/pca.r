generate_pca_projection_plots <- function(seed_vs_noseed_trajectories_rda_str, suffix=""){
	...
	noseed_points_per_task<- lapply(0:6, function(task_i){
		rr <- dcast(seed_vs_noseed_trajectories_rda_str[seed_vs_noseed_trajectories_rda_str$task_index==task_i & seed_vs_noseed_trajectories_rda_str$seed_id=="Not Seeded",], muscle_trajectory~muscle,value.var="activation")
	...
	p1 <- ggplot(normalized_loadings[normalized_loadings$Var2 %in% c("PC1","PC2","PC3"),], aes(task_index,value, col=Var2)) + geom_path() + facet_grid(Var2~Var1) + theme_classic() + ylab("loading value")
	...
	seeded_points_per_task <- lapply(0:6, function(task_i){
		ddf <- dcast(seed_vs_noseed_trajectories_rda_str[seed_vs_noseed_trajectories_rda_str$task_index==task_i & seed_vs_noseed_trajectories_rda_str$seed_id==input_seed_id,], muscle_trajectory~muscle,value.var="activation")
	...
	p <- p + geom_point(size=0.05, alpha=0.5,shape=1, col="grey", data=nonseed_and_seed_projected[nonseed_and_seed_projected$seed_id=="Not Seeded",]) 
	p <- p + geom_point(size=0.1, alpha=0.5,shape=1, data=nonseed_and_seed_projected[nonseed_and_seed_projected$seed_id!="Not Seeded",]) 
	p <- p + facet_grid(~task_index)
	p <- p + coord_fixed() 
	p <- p + theme_classic()
	p <- p + stat_chull(fill=NA, col="black")
	p <- p + theme(panel.grid = element_blank(), panel.border = element_blank())
	ggsave("outputs/PC_view_of_st_tunnel_%s"%--% suffix %>%time_dot("png"), p, width=20,height=20)
